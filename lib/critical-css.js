...
const shopPages = ['list-collections', 'collection', 'article', 'blog', 'page', 'product', 'search', 'index']

function printCriticalCssLiquid() {
	return `{% if template contains '${shopPages.listCollections}' %}\n  {% render 'critical-css-list-collections' %}\n
...
async function uploadShopifySnippets(shopifyAdmin, pages) {
	...
	const assetFetches = [];
	shopPages.forEach(async page => {
		const pageWithCss = pages.find(p => p.type === page);
		const snippet = pageWithCss && pageWithCss.error
			? '<!-- There was an error generating critical css for this page -->'
			: `<style id="critical-css-${page}">${pageWithCss ? pageWithCss.css : null}</style>`;

		assetFetches.push(shopifyAdmin.writeAsset({
			name: `snippets/critical-css-${page}.liquid`,
			value: snippet
		}));
	});
