const critical = require('critical');
const asyncPool = require("tiny-async-pool");
const getShopUrls = require('./getShopUrls');

require('events').EventEmitter.defaultMaxListeners = 50;

const dimensions = [{
	height: 640,	// moto G4
	width: 360
},
{
	height: 1024, // ipad
	width: 768
},
{
	height: 750, // macbook pro(ish)
	width: 1200
}];


function generateForPage(shopUrl) {
	console.log(`Generating critical css for url: ${shopUrl.url}...`)
	return critical.generate({
		src: shopUrl.url,
		dimensions: dimensions,
		extract: true,
		minify: true,
		ignore: {
			atrule: ['@font-face', '@keyframes', '@-moz-keyframes', '@-webkit-keyframes'],
			decl: (node, value) => /url\(/.test(value),
		}
	}).then(({css, html}) => {
		console.log(`DONE generating critical CSS for ${shopUrl.url}`);
		return {
			type: shopUrl.type,
			css: css
		}
	})
}

function printStyles(css) {
	let elseIf = '';
	let firstIf = '';
	let lastElse = '';
	css.forEach(page => {
		if(page.type === 'index') {
			lastElse = `{% else %}\n  <style id="critical-css-${page.type}">${page.css}</style>\n{% endif %}`;
		}
		else if(page.type === 'list-collections') {
			firstIf = `{% if template contains 'list-collections' %}\n  <style id="critical-css-${page.type}">${page.css}</style>\n`;
		}
		else {
			elseIf += `{% elsif template contains '${page.type}' %}\n  <style id="critical-css-${page.type}">${page.css}</style>\n`;
		}
	})
	return firstIf + elseIf + lastElse;
}

async function generateForShop(shop, job, cb, shopUrls) {
	const timeoutId = setTimeout(() => {
		throw new Error(`Timeout of ${process.env.TIMEOUT} exeeded for generating critical css`);
	}, process.env.TIMEOUT);

	shopUrls = shopUrls || await getShopUrls(shop);
	job.progress(20);
	const poolLimit = parseInt(process.env.CRITICAL_CSS_CONCURRENCY);
	const results = await asyncPool(poolLimit, shopUrls, generateForPage);
	console.log(`Generated critical css for all pages of ${shop}...`);
	job.progress(60);
	const criticalCss = printStyles(results);
	console.log('Generated text for critical-css.liquid...')
	job.progress(70);
	if(typeof cb === 'function') {
		cb(criticalCss);
	}
	clearTimeout(timeoutId);
	return criticalCss;
}

module.exports = {
	generateForShop,
	generateForPage
}
